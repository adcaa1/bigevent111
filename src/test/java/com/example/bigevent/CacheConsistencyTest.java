...
public class CacheConsistencyTest {
    ...
    public void updateUser_DelayedDoubleDelete(Integer userId, String newName) {
        ...
        redisTemplate.delete(USER_CACHE_KEY + userId);
        log.info("  Step 1: 第一次删缓存");

        // Step 2：更新数据库
        mockDB.put(userId, newName);
        log.info("  Step 2: 数据库更新成功 → " + newName);

        // Step 3：睡眠一段时间（模拟延迟，实际项目中根据业务耗时调整）
        log.info("  Step 3: 等待 500ms...（等可能读到旧值的线程把值写回缓存）");
        try {
            Thread.sleep(500);
        ...
        redisTemplate.delete(USER_CACHE_KEY + userId);
        log.info("  Step 4: 延迟 500ms 后第二次删缓存");
    }
    ...
    public void testCacheAsideConcurrentRisk() throws InterruptedException {
        log.info("\n===== 方案二并发风险说明（理论演示） =====");
        log.info("  这种时序极难发生，但理论上有风险：");
        log.info("  线程A：读缓存未命中 → 读数据库（拿到旧值） → [卡顿/网络延迟]");
        log.info("  线程B：更新数据库 → 删缓存");
        log.info("  线程A：[恢复] → 把旧值写入缓存  ← 此时缓存是脏数据");
        log.info("  ");
        log.info("  为什么极难发生？因为线程A的'读DB+写Cache'通常比线程B的'写DB'更快。");
        log.info("  万一真发生了怎么办？给缓存加过期时间（比如10分钟），过期后自动失效。");
    }
}
